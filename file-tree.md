...
│  │  │  ├─ VulkanVectorOverlayDepthPolicy.cs
...
- `XuanYu.Render.Vulkan/Pipeline/ShaderBytecode.Vert.cs` — F1-REWORK-B2：由 glslc -O 从 scene.vert 生成的 Vector Overlay Depth Policy 字节码。
...
- `XuanYu.Render.Vulkan/Render/VectorOverlay/VulkanClearFrameOwner.DrawVectorOverlay.cs` — F1-V1/B2：Vector Overlay Vulkan Pass 绘制、DIP 尺寸与 primitive depth mode 注入。
...
- `XuanYu.Render.Vulkan/Render/VectorOverlay/VulkanVectorOverlayDepthPolicy.cs` — F1-REWORK-B2：Fill / Stroke / Marker 的有界裁剪空间深度偏移策略。
