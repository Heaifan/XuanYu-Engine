...
// VK4-D：薄组合根。只装配 ClearFrame + PresentLoop（+ VK5-A GraphicsPipeline），不写 Vulkan 细节。
// Bridge 仅委托 Attach/Resize/Detach；Detach 顺序 Pipeline → ClearFrame → Swapchain → ...（由 Bridge 保证后续）。
public sealed class VulkanRenderSession : IDisposable
{
    ...
    VulkanRenderSession(VulkanDeviceOwner deviceOwner, VulkanSwapchainOwner swapchainOwner,
        VulkanClearFrameOwner clearFrame, VulkanPresentLoop presentLoop, VulkanGraphicsPipelineOwner? pipeline, Action<string>? log)
    {
        _deviceOwner = deviceOwner; _swapchainOwner = swapchainOwner; _clearFrame = clearFrame; _presentLoop = presentLoop; _pipeline = pipeline; _log = log;
    }
        ...
        VulkanSwapchainOwner? swapchainOwner, VulkanPhysicalDeviceSelection? selection, Action<string>? log)
    {
        ...
        try
        {
            ...
            return new VulkanRenderSession(deviceOwner, swapchainOwner, clear, loop, pipeline, log);
        }
