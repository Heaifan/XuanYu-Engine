...
// MAP-A-R1-D5-R1-F2-R2：全屏 Pass 管线组合（参考网格 / 世界轴 / 世界原点）。
// 设备不支持时对应 Pass 禁用（返回 null），不阻止编辑器启动。
internal sealed class GridPipelineSet : IDisposable
{
    GridPipelineSet(VulkanGraphicsPipelineOwner? grid, VulkanGraphicsPipelineOwner? axes, VulkanGraphicsPipelineOwner? origin)
    {
        Grid = grid;
        Axes = axes;
        Origin = origin;
        ...
        VulkanClearFrameOwner clear, VulkanSwapchainOwner swapchain, PhysicalDevice physicalDevice, Action<string>? log)
    {
        ...
        return new GridPipelineSet(grid, axes, origin);
    }
