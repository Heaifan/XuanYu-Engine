...
public sealed class ScenePointerPickerTests
{
    ...
    {
        // Entity at (0, 0.5, 0) with 1.25 bounds, ray from above
        var id = EntityId.FromInt(1);
        var scene = MakeSceneWithEntity(id, new Vector3d(0, 0.5, 0), 0.625, RenderObjectVisualKind.UnitMarker);
    ...
    {
        // Entity at (100, 0.5, 0) — far away
        var id = EntityId.FromInt(1);
        var scene = MakeSceneWithEntity(id, new Vector3d(100, 0.5, 0), 0.625, RenderObjectVisualKind.UnitMarker);

        // Ray through the origin downward
        var ray = new SceneRay(new Vector3d(0, 10, 0), new Vector3d(0, -1, 0));
        var result = ScenePointerPicker.Pick(ray, scene, DefaultGround);
    ...
    {
        // Two entities, one at (0,0.5,0) and another at (5,0.5,0)
        var id1 = EntityId.FromInt(1);
        var id2 = EntityId.FromInt(2);
        var bounds1 = new SceneAxisAlignedBounds(new Vector3d(0, 0.5, 0), new Vector3d(0.625, 0.625, 0.625));
        var obj1 = new RenderObjectInfo(id1, "near", new Vector3d(0, 0.5, 0),
            RenderObjectVisualKind.UnitMarker, "a.json", bounds1);
        var bounds2 = new SceneAxisAlignedBounds(new Vector3d(5, 0.5, 0), new Vector3d(0.625, 0.625, 0.625));
        var obj2 = new RenderObjectInfo(id2, "far", new Vector3d(5, 0.5, 0),
            RenderObjectVisualKind.UnitMarker, "b.json", bounds2);
        var scene = new RenderScene([obj1, obj2]);

        // Ray through (0,0,0) downward
        var ray = new SceneRay(new Vector3d(0, 10, 0), new Vector3d(0, -1, 0));
        var result = ScenePointerPicker.Pick(ray, scene, DefaultGround);
