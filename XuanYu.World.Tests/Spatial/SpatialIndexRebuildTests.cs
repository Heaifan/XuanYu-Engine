...
public sealed class SpatialIndexRebuildTests
{
    [Fact]
    public void Rebuild_from_world_keeps_radius_and_bounds_results()
    {
        ...
        SpatialQueryOracle.AssertSame(beforeRadius, world.QueryRadius(center, radius));
        SpatialQueryOracle.AssertSame(beforeBounds, world.QueryBounds(bounds));
    ...
    public void Deterministic_random_queries_match_oracle_after_random_moves()
    {
        ...
        for (var i = 0; i < 200; i++)
        {
            ...
            var bounds = SpatialQueryOracle.BoundsAround(center, radius);
            SpatialQueryOracle.AssertSame(SpatialQueryOracle.Radius(world, center, radius), world.QueryRadius(center, radius));
            SpatialQueryOracle.AssertSame(SpatialQueryOracle.Bounds(world, bounds), world.QueryBounds(bounds));
        }
    }
