...
public sealed class WorldCR4D4LoadTransactionTests : IDisposable
{
    ...
    {
        var glb = _env.NewGlb("soldier", WorldCR4D1GlbFactory.Triangle());
    ...
    {
        var glb1 = _env.NewGlb("a", WorldCR4D1GlbFactory.Triangle());
        var glb2 = _env.NewGlb("b", WorldCR4D1GlbFactory.MultiPrimitive());
        _env.ImportGlb(glb1);
        _env.ImportGlb(glb2);
        var scenePath = _env.NewScenePath();
        await _env.SaveAsync(scenePath);
