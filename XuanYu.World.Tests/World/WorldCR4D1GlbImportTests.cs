...
public sealed class WorldCR4D1GlbImportTests
{
    ...
    {
        var result = Import(WorldCR4D1GlbFactory.Triangle());
    ...
    {
        var result = Import(WorldCR4D1GlbFactory.Triangle(uvs: false));
        Assert.True(result.Succeeded);
    ...
    {
        var result = Import(WorldCR4D1GlbFactory.Triangle(indices: false));
        Assert.True(result.Succeeded);
        Assert.Equal([0u, 1u, 2u], result.Model!.Indices);
    }
    ...
    {
        var result = Import(WorldCR4D1GlbFactory.MultiPrimitive());
        Assert.True(result.Succeeded);
    ...
    {
        var result = Import(WorldCR4D1GlbFactory.Triangle(translation: [1, 2, 3]));
        Assert.True(result.Succeeded);
    ...
    {
        var result = Import(WorldCR4D1GlbFactory.Triangle(mode: 0));
    ...
    public void Required_extension_fails_before_data_escape()
    {
        ...
        var result = Import(WorldCR4D1GlbFactory.Triangle(extraRoot: root));
        Assert.False(result.Succeeded);
    ...
    {
        var result = Import(WorldCR4D1GlbFactory.InvalidHeader());
        Assert.False(result.Succeeded);
