...
public sealed class WorldCR4D3AuthoringServiceTests
{
    ...
    {
        using var file = TempGlb(WorldCR4D1GlbFactory.InvalidHeader());
        var (scene, catalog) = NewScene();
        var before = scene.Entities.Count;
        var result = _service.Import(file.Path, scene, catalog);
        Assert.False(result.Succeeded);
    ...
    static TempFile TempGlb(byte[]? bytes = null)
    {
        ...
        File.WriteAllBytes(path, bytes ?? WorldCR4D1GlbFactory.Triangle());
