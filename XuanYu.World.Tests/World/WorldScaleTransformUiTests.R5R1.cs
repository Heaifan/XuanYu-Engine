...
{
    [Fact]
    public void R5R1_y_and_z_axis_still_scale_only_their_component()
    {
    ...
    [Fact]
    public void R5R1_uniform_drag_commits_once_and_undo_redo_restores()
    {
        var vm = ScaleVm();
    ...
    [Fact]
    public void R5R1_uniform_escape_cancel_rejects_late_mouse_up()
    {
        var vm = ScaleVm();
        var (hx, hy, vp) = ScaleHit(vm, ScaleGizmoHandle.Uniform);
        var (px, py) = ScalePull(vm, ScaleGizmoHandle.Uniform, vp, 110);
