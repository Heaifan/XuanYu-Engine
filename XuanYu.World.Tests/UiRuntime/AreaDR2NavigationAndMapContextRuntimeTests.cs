...
public sealed class AreaDR2NavigationAndMapContextRuntimeTests
{
    ...
    public void Map_context_renders_map_inspector_asset_and_layer_once()
    {
        ...
        var state = host.Run(() =>
        {
            var vm = new UiVm(null, seedInitialScene: false); vm.SwitchWorkspaceCommand.Execute("MapEditor"); vm.ToggleEditorMode(); vm.MapSession.SelectMap(); vm.ToggleInspectorSectionCommand.Execute(InspectorSectionId.Basic);
    ...
    public void Clearing_entity_owner_restores_single_map_context()
    {
        ...
        var state = host.Run(() =>
        {
            ...
            vm.SelectedHierarchyItem = null; vm.MapSession.SelectMap(); vm.ToggleInspectorSectionCommand.Execute(InspectorSectionId.Basic); Dispatcher.UIThread.RunJobs();
            var right = new Right { DataContext = vm }; host.Show(right, 480, 720); Dispatcher.UIThread.RunJobs();
            UiRuntimeTestHost.Descendants<XYTabs>(right).Single().Select("inspector"); right.UpdateLayout();
            return Snapshot(right);
        });
