...
public sealed class XYUI3ViewSwitcherTests : IClassFixture<XyuiHeadlessFixture>
{
    ...
    [Fact] public void Request_requires_accept_and_rejected_request_keeps_current() => _fx.Run(() => { XyuiBatchTestHost.Prepare(); var switcher = new XYViewSwitcher(new XYViewState(Views, "canvas")); switcher.SelectView("table"); Assert.Equal("canvas", switcher.CurrentViewId); switcher.ViewChangeRequested += (_, request) => request.Reject(); switcher.SelectView("table"); Assert.Equal("canvas", switcher.CurrentViewId); switcher.ViewChangeRequested += (_, request) => request.Accept(); switcher.SelectView("table"); Assert.Equal("table", switcher.CurrentViewId); });
