...
public sealed class XYUI3StructureTests : IClassFixture<XyuiHeadlessFixture>
{
    ...
    [Fact] public void SubMenu_reuses_XYMenu_and_XYMenuItem() => _fx.Run(() => { XyuiBatchTestHost.Prepare(); var parent = new XYMenu(new XYMenuItem { Label = "导出", HasSubMenu = true }); var child = new XYMenu(new XYMenuItem { Label = "导出图片" }); var submenu = new XYSubMenu { ParentMenu = parent, ChildMenu = child }; Assert.Same(parent, submenu.ParentMenu); Assert.Same(child, submenu.ChildMenu); Assert.Single(submenu.GetVisualDescendants().OfType<XYMenu>()); Assert.DoesNotContain(parent, submenu.GetVisualDescendants()); });
