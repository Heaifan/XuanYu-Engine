...
public sealed class XYUI3StructureTests : IClassFixture<XyuiHeadlessFixture>
{
    ...
    [Fact] public void Gallery_registers_only_batch01_pages() => _fx.Run(() => { XyuiBatchTestHost.Prepare(); var docs = XYUI3DocumentationCatalog.Build(); Assert.Equal(5, docs.Count); Assert.All(docs.Where(x => x.Id != "XYUI-3-3.04"), x => Assert.Contains("AWAITING USER VISUAL ACCEPTANCE", x.Acceptance)); Assert.Equal("UI CLOSED · USER VISUAL ACCEPTED · HIERARCHY LOGIC REWORKED · AWAITING USER INTERACTION VERIFICATION", docs.Single(x => x.Id == "XYUI-3-3.04").Acceptance); Assert.All(docs, x => Assert.NotNull(x.PreviewFactory())); });
