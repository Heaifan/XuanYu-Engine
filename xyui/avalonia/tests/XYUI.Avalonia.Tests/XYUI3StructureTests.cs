...
public sealed class XYUI3StructureTests : IClassFixture<XyuiHeadlessFixture>
{
    ...
    [Fact] public void Gallery_registers_only_batch01_pages() => _fx.Run(() => { XyuiBatchTestHost.Prepare(); var docs = XYUI3DocumentationCatalog.Build(); Assert.Equal(4, docs.Count); Assert.All(docs, x => Assert.Contains("AWAITING USER VISUAL ACCEPTANCE", x.Acceptance)); Assert.All(docs, x => Assert.NotNull(x.PreviewFactory())); });
}
