...
public sealed class XYUI2PropertyControlsTests : IClassFixture<XyuiHeadlessFixture>
{
    ...
    {
        XyuiBatchTestHost.Prepare(); var p = new XYVectorProperty { Width = 620, Dimension = XYVectorDimension.Vector3, X = 1, Y = 2, Z = 3 }; var w = XyuiBatchTestHost.Show(p);
        ...
        Assert.Equal(1, p.X); Assert.Equal(8, p.Y); Assert.Equal(3, p.Z); Assert.True(y.IsScrubEnabled); p.Dimension = XYVectorDimension.Vector2; Assert.False(p.AxisHosts[2].IsVisible); w.Close();
    });
