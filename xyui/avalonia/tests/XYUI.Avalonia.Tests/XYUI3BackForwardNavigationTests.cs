...
public sealed class XYUI3BackForwardNavigationTests : IClassFixture<XyuiHeadlessFixture>
{
    ...
    [Fact] public void Surface_owns_content_and_compact_geometry_is_real() => _fx.Run(() =>
    {
        ...
        var locationText = nav.GetVisualDescendants().OfType<TextBlock>().Single(x => x.Classes.Contains("xyui-location-text")); var location = Assert.IsType<StackPanel>(locationText.GetVisualParent()); Assert.Equal(34, nav.Height); Assert.Equal(34, surface.Height); Assert.True(nav.Bounds.Width >= 200); Assert.True(location.Bounds.Width >= 130); Assert.True(location.Bounds.Width <= 240); Assert.True(location.Margin.Left >= 8); Assert.Equal(28, nav.BackButton.Width); Assert.Equal(28, nav.ForwardButton.Height); Assert.Same(nav.BackButton, inner.Children[0]); Assert.Same(nav.ForwardButton, inner.Children[1]); Assert.Equal(VerticalAlignment.Center, nav.BackButton.VerticalAlignment); Assert.Equal(VerticalAlignment.Center, nav.ForwardButton.VerticalAlignment); window.Close();
    });
    ...
    {
        XyuiBatchTestHost.Prepare(); var nav = new XYBackForwardNavigation(); nav.Navigate("地图"); nav.Navigate("数据集"); nav.Navigate("roads"); nav.Navigate("广东省"); var window = XyuiBatchTestHost.Show(nav); var point = nav.BackButton.TranslatePoint(new Point(14, 14), window)!.Value; window.MouseDown(point, MouseButton.Right); Dispatcher.UIThread.RunJobs(); Assert.True(nav.BackHistoryPopup.IsOpen); Assert.All(nav.BackHistoryMenu.Items.Where(x => x is XYMenuItem), x => Assert.IsType<XYMenuItem>(x)); Assert.Equal("roads", ((XYMenuItem)nav.BackHistoryMenu.Items[0]).Label); Assert.Null(((XYMenuItem)nav.BackHistoryMenu.Items[0]).Icon); ((XYMenuItem)nav.BackHistoryMenu.Items[1]).Activate(); Assert.Equal("数据集", nav.CurrentLocation); Assert.Equal(1, nav.CurrentIndex); Assert.False(nav.BackHistoryPopup.IsOpen); window.Close();
    });
    ...
    [Fact] public void Long_location_trims_without_resizing_actions() => _fx.Run(() =>
    {
