...
public sealed class XYUI3Batch05StructureTests : IClassFixture<XyuiHeadlessFixture>
{
    ...
    {
        XyuiBatchTestHost.Prepare(); var host = XYUI3GalleryCatalog.CreatePreview("XYUI-3-3.17"); var bar = Assert.IsType<XYCommandBar>(host.GetVisualDescendants().First()); var count = 0; bar.CommandExecuted += (_, _) => count++; var first = bar.Items[0]; first.RaiseEvent(new global::Avalonia.Interactivity.RoutedEventArgs(Button.ClickEvent)); Assert.Equal(1, count); Assert.True(first.IsSelected); Assert.Same(first, bar.SelectedItem); Assert.Contains("xyui-command-selected", first.Classes); var second = bar.Items[1]; second.RaiseEvent(new global::Avalonia.Interactivity.RoutedEventArgs(Button.ClickEvent)); Assert.False(first.IsSelected); Assert.True(second.IsSelected); Assert.Same(second, bar.SelectedItem); bar.MoreButton.RaiseEvent(new global::Avalonia.Interactivity.RoutedEventArgs(Button.ClickEvent)); Assert.True(bar.MorePopup.IsOpen); Assert.Equal(34, bar.Height); Assert.Equal(XYCommandRole.Primary, first.Role); Assert.Equal(XyuiVectorIcon.Add, first.Icon);
    });

    [Fact] public void CommandBar_many_commands_do_not_overlap() => _fx.Run(() =>
    {
        XyuiBatchTestHost.Prepare(); var bar = new XYCommandBar(Enumerable.Range(1, 8).Select(i => new XYCommandItem($"命令{i}")).ToArray()); Assert.Equal(8, bar.Child!.GetVisualDescendants().OfType<XYCommandItem>().Count()); Assert.Empty(bar.Child!.GetVisualDescendants().OfType<Canvas>());
    });

    [Fact] public void CommandBar_more_escape_closes_and_context_replaces_commands() => _fx.Run(() =>
    {
        XyuiBatchTestHost.Prepare(); var bar = new XYCommandBar(new XYCommandItem("编辑")) { }; bar.MoreMenu.Items = [new XYMenuItem { Label = "导出" }]; bar.RefreshMore(); bar.MoreButton.RaiseEvent(new global::Avalonia.Interactivity.RoutedEventArgs(Button.ClickEvent)); Assert.True(bar.MorePopup.IsOpen); bar.MoreButton.RaiseEvent(new KeyEventArgs { RoutedEvent = InputElement.KeyDownEvent, Key = Key.Escape }); Assert.False(bar.MorePopup.IsOpen); var contextual = new XYCommandBar(XYCommandBarVariant.Contextual, "roads", new XYCommandItem("编辑")); contextual.UpdateContext("cities", new XYCommandItem("复制")); Assert.Equal("cities", contextual.ContextIdentity); Assert.Equal("复制", contextual.Items.Single().Content is TextBlock text ? text.Text : "");
    });
