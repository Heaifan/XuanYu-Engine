...
public static partial class XYUI3GalleryCatalog
{
    ...
    static Control TreeNavigationPreview() => new XYTreeNavigation(
        Node("玄域项目", 0, XyuiVectorIcon.Section, children: true),
        Node("地图", 1, XyuiVectorIcon.Locate, children: true, active: 1),
        Node("数据集", 2, XyuiVectorIcon.Section, children: true, active: 2),
    ...
    static XYTreeNode Node(string label, int depth, XyuiVectorIcon icon, bool children = false, bool selected = false, int active = 0) =>
        new() { Label = label, Depth = depth, Icon = icon, HasChildren = children, IsSelected = selected, ActiveGuideDepth = active };
}
