...
public static partial class XYUI3GalleryCatalog
{
    ...
    static Control CommandBarPreview() { var bar = new XYCommandBar(new XYCommandItem("新建", "new"), new XYCommandItem("导入", "import"), new XYCommandItem("保存", "save"), new XYCommandItem("验证", "validate"), new XYCommandItem("删除", "delete")); bar.MoreMenu.Items = [Item("复制"), Item("重命名")]; return bar; }
    static Control CommandPalettePreview() => new XYCommandPalette(new XYPaletteCommand("创建道路", "地图"), new XYPaletteCommand("验证道路"), new XYPaletteCommand("打开道路数据集"), new XYPaletteCommand("进入道路工具")) { Width = 440 };
    ...
    static Control WorkspacePreview() => new XYWorkspaceSwitcher("地图编辑", "地图编辑", "数据编辑", "战争实验", "调试分析");
}
