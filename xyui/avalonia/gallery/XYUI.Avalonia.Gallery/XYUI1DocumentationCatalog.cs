...
public static partial class XYUI1DocumentationCatalog
{
    ...
    static (string Overview, string WhenToUse) Details(string id, string title) => id switch
    {
        ...
        "XYUI-1-07" => ("代码 / ID 使用等宽字体强调可复制、可比对的技术值。", "用于实体 ID、路径、短代码和诊断信息。"),
        "XYUI-1-08" => ("等宽数据让坐标、数值和结构化短值保持稳定的字符节奏。", "用于坐标、尺寸、哈希和调试数据。"),
        "XYUI-1-09" => ("标签是紧凑的分类或属性标记，支持默认和强调两种视觉变体。", "用于类型、筛选条件和轻量分类信息。"),
        "XYUI-1-10" => ("状态标签把状态语义与短文本放在同一个可读单元中。", "用于保存、连接、校验和同步状态。"),
        "XYUI-1-11" => ("状态圆点用最小面积传达状态，不承担完整状态文字。", "用于列表、树节点或状态标签旁的快速指示。"),
        "XYUI-1-12" => ("图标是单个符号的轻量承载，尺寸与 StrokeWidth 按 Tiny / Small / Default / Large 联动。当前 glyph registry 缺口仍登记为 GAP-001。", "用于动作提示、状态补充和紧凑工具栏。"),
        ...
        "XYUI-1-23" => ("搜索高亮突出检索命中词，同时保留原始文本的可读性。", "用于搜索结果、树节点和属性列表中的命中内容。"),
