...
public static partial class XyuiComponentStyles
{
    ...
    {
        MenuBar(styles); Menu(styles); ContextMenu(styles); SubMenu(styles); MenuText(styles); NavigationMenu(styles); SidebarRailTabs(styles); TabBarDockTabs(styles); BreadcrumbTree(styles); Batch04(styles);
    }
    static void MenuBar(Styles styles)
    {
        ...
        item.Setters.Add(new Setter(Border.HeightProperty, 35d)); item.Setters.Add(new Setter(Border.PaddingProperty, new Thickness(10, 0))); item.Setters.Add(new Setter(Border.CornerRadiusProperty, new CornerRadius(3))); item.Setters.Add(new Setter(Border.BackgroundProperty, Brushes.Transparent)); styles.Add(item);
