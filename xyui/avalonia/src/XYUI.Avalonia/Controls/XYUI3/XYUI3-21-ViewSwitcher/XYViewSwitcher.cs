...
public sealed class XYViewChangeRequest : EventArgs
{
    ...
    public void Accept() { if (IsRejected) return; IsAccepted = true; } public void Reject() { IsAccepted = false; IsRejected = true; }
}
...
public sealed partial class XYViewSwitcher : Border
{
    ...
    public XYViewSwitcher(XYViewState state, XYViewSwitcherVariant variant = XYViewSwitcherVariant.Segmented) { State = state; Variant = variant; if (variant == XYViewSwitcherVariant.Dropdown) Width = 220; State.Changed += (_, _) => Refresh(); Classes.Add("xyui-view-switcher"); _popup.Child = _menu; _popup.Closed += (_, _) => ClosePopup(); _menu.Closed += (_, _) => _popup.IsOpen = false; Child = _host; Refresh(); }
    ...
    XYButton BuildTrigger()
    {
        var current = Views.FirstOrDefault(v => v.Id == CurrentViewId); var grid = new Grid { ColumnDefinitions = new ColumnDefinitions("Auto,*,Auto"), VerticalAlignment = VerticalAlignment.Center };
        var label = new TextBlock { Text = "视图", Classes = { "xyui-view-dropdown-label" }, VerticalAlignment = VerticalAlignment.Center };
        var value = new TextBlock { Text = current?.Label ?? "选择视图", Classes = { "xyui-view-dropdown-current" }, VerticalAlignment = VerticalAlignment.Center, Margin = new Thickness(12, 0, 0, 0) };
        var chevron = new XYIcon { Icon = XyuiVectorIcon.ChevronDown, Size = XyuiIconSize.Small, VerticalAlignment = VerticalAlignment.Center }; Grid.SetColumn(value, 1); Grid.SetColumn(chevron, 2); grid.Children.Add(label); grid.Children.Add(value); grid.Children.Add(chevron);
        var button = new XYButton { Height = 34, Width = 220, HorizontalAlignment = HorizontalAlignment.Stretch, HorizontalContentAlignment = HorizontalAlignment.Stretch, Variant = XyuiButtonVariant.Secondary, Classes = { "xyui-view-dropdown-trigger" }, Content = grid }; button.Click += (_, _) => Open(button); return button;
    }
    void BuildPrimaryMore(StackPanel host) { var primary = Views.Where(v => v.Priority >= 0).OrderByDescending(v => v.Priority).Take(3).ToArray(); foreach (var view in primary) host.Children.Add(BuildSegment(view)); var rest = Views.Except(primary).ToArray(); if (rest.Length > 0) { host.Children.Add(new XYSeparator { Variant = XyuiSeparatorVariant.VerticalSplit, Height = 20, Margin = new Thickness(4, 0), Classes = { "xyui-view-more-divider" } }); var more = new XYIconButton { Content = new XYIcon { Icon = XyuiVectorIcon.MoreHorizontal, Size = XyuiIconSize.Small }, Classes = { "xyui-view-more" }, IsSelected = rest.Any(v => v.Id == CurrentViewId) }; more.Click += (_, _) => Open(more, rest); host.Children.Add(more); } }
