...
public sealed partial class XYTabs : Border
{
    ...
    public void Build()
    {
        ...
        foreach (var item in Items) { item.SelectionRequested -= OnSelected; item.SelectionRequested += OnSelected; item.CloseRequested -= OnCloseRequested; item.CloseRequested += OnCloseRequested; _panel.Children.Add(item); }
    }
}
