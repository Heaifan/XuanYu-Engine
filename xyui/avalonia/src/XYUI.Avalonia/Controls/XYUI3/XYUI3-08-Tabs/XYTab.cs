...
public sealed class XYTab : Border
{
    ...
    void Build() { Classes.Set("xyui-tab-selected", IsSelected); Child = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 7, Children = { new TextBlock { Text = Label, Classes = { "xyui-tab-label" } }, new Border { Classes = { "xyui-tab-modified" }, IsVisible = IsModified }, new Button { Content = "×", Classes = { "xyui-tab-close" }, IsVisible = false } } }; PointerPressed += (_, _) => { IsSelected = true; Selected?.Invoke(this, EventArgs.Empty); }; }
}
