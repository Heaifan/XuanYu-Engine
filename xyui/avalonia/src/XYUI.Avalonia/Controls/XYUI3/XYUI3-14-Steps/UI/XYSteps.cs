...
public sealed class XYSteps : Border
{
    ...
    void BuildVertical()
    {
        ...
        for (var i = 0; i < Items.Count; i++) { _cells.RowDefinitions.Add(new RowDefinition(1, GridUnitType.Auto)); Items[i].SetVertical(true); Grid.SetRow(Items[i], i); Grid.SetColumnSpan(Items[i], 2); Items[i].HorizontalAlignment = HorizontalAlignment.Stretch; _cells.Children.Add(Items[i]); }
        for (var i = 0; i + 1 < Items.Count; i++) _connectors.Add(AddConnector());
    }
    Border AddConnector()
    {
