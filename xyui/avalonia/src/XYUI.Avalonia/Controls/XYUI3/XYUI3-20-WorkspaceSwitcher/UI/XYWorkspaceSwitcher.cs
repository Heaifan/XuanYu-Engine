...
public sealed class XYWorkspaceSwitcher : Border
{
    ...
    {
        CurrentWorkspace = current; Workspaces = workspaces.Length == 0 ? [current] : workspaces; Classes.Add("xyui-workspace-switcher"); Trigger.Click += (_, _) => Toggle(); _popup.Child = _items; Child = Build(); Refresh();
    }
