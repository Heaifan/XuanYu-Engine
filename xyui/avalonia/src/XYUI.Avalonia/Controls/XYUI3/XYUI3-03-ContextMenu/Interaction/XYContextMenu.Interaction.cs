...
public sealed partial class XYContextMenu
{
    ...
    public void AttachTo(Control source) => source.PointerPressed += OnSourcePointerPressed;
    public void Open(Control target)
    {
        if (!IsEnabled) return; Close(); _host.Children.Remove(_surface); _popup = new Popup { PlacementTarget = target, Placement = PlacementMode.Pointer, IsLightDismissEnabled = true, Child = _surface };
        _popup.Closed += OnPopupClosed; IsOpen = true; _popup.IsOpen = true; Opened?.Invoke(this, EventArgs.Empty);
    }
    public void Close()
    {
        if (!IsOpen && _popup is null) return; IsOpen = false; if (_popup is not null) { _popup.Closed -= OnPopupClosed; _popup.IsOpen = false; _popup.Child = null; _popup = null; }
        if (!_host.Children.Contains(_surface)) _host.Children.Insert(0, _surface); Closed?.Invoke(this, EventArgs.Empty);
    }
