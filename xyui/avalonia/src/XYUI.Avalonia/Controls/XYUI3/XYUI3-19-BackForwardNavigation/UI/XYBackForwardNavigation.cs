...
public sealed class XYBackForwardNavigation : Border
{
    ...
    Control Build() { var grid = new Grid { Width = 500, Height = 60, ColumnDefinitions = new ColumnDefinitions("34,34,1,*") }; var surface = new Border { Margin = new global::Avalonia.Thickness(10, 12), Classes = { "xyui-back-forward-surface" } }; Grid.SetColumnSpan(surface, 4); grid.Children.Add(surface); Grid.SetColumn(BackButton, 0); Grid.SetColumn(ForwardButton, 1); grid.Children.Add(BackButton); grid.Children.Add(ForwardButton); var location = new StackPanel { Margin = new global::Avalonia.Thickness(14, 0, 0, 0), VerticalAlignment = VerticalAlignment.Center }; location.Children.Add(new TextBlock { Text = "当前位置", Classes = { "xyui-location-title" } }); location.Children.Add(new TextBlock { Text = CurrentLocation ?? "roads / 道路编辑", Classes = { "xyui-location-text" } }); Grid.SetColumn(location, 3); grid.Children.Add(location); return grid; }
