...
public sealed class XYBackForwardNavigation : Border
{
    ...
    readonly TextBlock _location = new() { Classes = { "xyui-location-text" } };
    ...
    Control Build() { var grid = new Grid { Width = 500, Height = 60, ColumnDefinitions = new ColumnDefinitions("34,34,1,*") }; var surface = new Border { Margin = new Thickness(10, 12), Classes = { "xyui-back-forward-surface" } }; Grid.SetColumnSpan(surface, 4); grid.Children.Add(surface); Grid.SetColumn(BackButton, 0); Grid.SetColumn(ForwardButton, 1); grid.Children.Add(BackButton); grid.Children.Add(ForwardButton); var divider = new Border { Width = 1, Height = 20, Classes = { "xyui-back-forward-divider" } }; Grid.SetColumn(divider, 2); grid.Children.Add(divider); var location = new StackPanel { Margin = new Thickness(14, 0, 0, 0), VerticalAlignment = VerticalAlignment.Center }; location.Children.Add(new TextBlock { Text = "当前位置", Classes = { "xyui-location-title" } }); location.Children.Add(_location); Grid.SetColumn(location, 3); grid.Children.Add(location); return grid; }
    ...
    void Sync() { BackButton.IsEnabled = CanGoBack; ForwardButton.IsEnabled = CanGoForward; _location.Text = CurrentLocation ?? "roads / 道路编辑"; }
