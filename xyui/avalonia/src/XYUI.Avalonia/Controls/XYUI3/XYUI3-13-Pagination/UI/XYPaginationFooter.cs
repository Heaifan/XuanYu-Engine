...
public sealed class XYPaginationFooter : Border
{
    ...
    public XYPaginationFooter(int totalItems = 468, int totalPages = 24)
    {
        ...
        PageSize.SelectionChanged += (_, _) => { var size = PageSize.SelectedItem is int value ? value : 25; Pagination.TotalPages = Math.Max(1, (int)Math.Ceiling(totalItems / (double)size)); Pagination.GoTo(Math.Min(Pagination.CurrentPage, Pagination.TotalPages)); };
    }
}
