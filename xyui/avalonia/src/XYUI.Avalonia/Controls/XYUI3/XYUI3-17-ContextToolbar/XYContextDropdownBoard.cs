...
public sealed class XYContextDropdownBoard : Border
{
    ...
    XYContextOverlayHost? _overlayHost; Control? _anchor; Point _rootPosition; bool _open;
    ...
    void BuildSurface()
    {
        _rootSurface.Child = new StackPanel { Spacing = 4, Children = { new TextBlock { Text = Header, Classes = { "xyui-context-menu-header" } }, Menu, new XYSeparator(), HintBar } };
        for (var i = 0; i < SubMenus.Count; i++) BuildChildSurface(i, SubMenus[i]);
    }
    void BuildChildSurface(int index, XYSubMenu submenu)
    {
        var surface = new Border { Width = 142, IsVisible = false, Classes = { "xyui-context-menu-surface" }, Child = new StackPanel { Spacing = 4, Children = { new TextBlock { Text = ((XYMenuItem)Menu.Items[index]).Label, Classes = { "xyui-context-menu-header" } }, submenu.ChildMenu } } };
        _childSurfaces.Add(surface); submenu.Opened += (_, _) => { surface.IsVisible = true; Dispatcher.UIThread.Post(() => PlaceChild(index), DispatcherPriority.Render); }; submenu.Closed += (_, _) => surface.IsVisible = false;
    }
    void PlaceRoot()
    {
    ...
    void PlaceChild(int index)
    {
        ...
        if (x + size.Width > owner.Width) x = _rootPosition.X - size.Width - 5; var trigger = Menu.Items.OfType<XYMenuItem>().ElementAt(index); var point = trigger.TranslatePoint(new Point(0, 0), _overlayHost); var y = point?.Y ?? _rootPosition.Y; x = Math.Clamp(x, 0, Math.Max(0, owner.Width - size.Width)); y = Math.Clamp(y, 0, Math.Max(0, owner.Height - size.Height)); Canvas.SetLeft(surface, x); Canvas.SetTop(surface, y);
    }
    void OnOverlaySizeChanged(object? sender, SizeChangedEventArgs e) => Dispatcher.UIThread.Post(PlaceRoot, DispatcherPriority.Render);
