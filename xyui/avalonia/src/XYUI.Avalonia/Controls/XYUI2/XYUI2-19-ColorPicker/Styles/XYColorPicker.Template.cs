...
public partial class XYColorPicker
{
    ...
    void SyncVisuals() { if (ValuePart is not null) ValuePart.Text = DisplayValue(); if (SwatchPart is not null) SwatchPart.Background = new SolidColorBrush(Color); if (ChevronPart is not null) ChevronPart.RenderTransform = new RotateTransform(IsOpen ? 180 : 0); SyncPanelValues(); SyncModeVisibility(); }
}
