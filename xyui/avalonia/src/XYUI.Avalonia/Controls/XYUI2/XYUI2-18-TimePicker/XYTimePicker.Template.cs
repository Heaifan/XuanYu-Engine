...
public partial class XYTimePicker
{
    internal static FuncControlTemplate<XYTimePicker> CreateTemplate() => new((control, scope) =>
    {
        ...
        var surface = new Grid { ColumnDefinitions = new ColumnDefinitions { new(1, GridUnitType.Star), new(32, GridUnitType.Pixel) }, Children = { segments, glyphCell } }; Grid.SetColumn(glyphCell, 1); surface.AddHandler(InputElement.PointerPressedEvent, control.OnSurfacePointerPressed, RoutingStrategies.Bubble, true);
        var chrome = new Border { Name = "PART_Chrome", Child = surface }; chrome[!Border.BackgroundProperty] = control[!TemplatedControl.BackgroundProperty]; chrome[!Border.BorderBrushProperty] = control[!TemplatedControl.BorderBrushProperty]; chrome[!Border.BorderThicknessProperty] = control[!TemplatedControl.BorderThicknessProperty]; chrome[!Border.CornerRadiusProperty] = control[!TemplatedControl.CornerRadiusProperty]; chrome.AddHandler(InputElement.PointerPressedEvent, control.OnSurfacePointerPressed, RoutingStrategies.Bubble, true); var edge = new Border { Name = "PART_FocusEdge", Height = 3, VerticalAlignment = VerticalAlignment.Bottom, IsHitTestVisible = false }; scope?.Register("PART_Chrome", chrome); scope?.Register("PART_FocusEdge", edge); return new Grid { Children = { chrome, edge } };
    });
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e) { base.OnApplyTemplate(e); SegmentButtons.Clear(); SecondSeparatorPart = e.NameScope.Find<Button>("PART_SecondSeparator"); ScrubIndicatorPart = e.NameScope.Find<XYIcon>("PART_ScrubIndicator"); SyncTimeParts(); }
    Button CreateSegment(XYTimeSegment segment, INameScope? scope) { var button = new XYTimeSegmentButton { Name = $"PART_{segment}", Content = SegmentText(segment), Padding = new Thickness(2, 0), Template = XyuiControlStyles.ActionCellTemplate(), Background = Brushes.Transparent, BorderThickness = new Thickness(0), Focusable = true, MinWidth = 26, IsVisible = segment != XYTimeSegment.Second || ShowSeconds, Pressed = e => BeginScrub(segment, e) }; button.Click += (_, _) => ActivateSegment(segment); SegmentButtons[segment] = button; scope?.Register(button.Name!, button); return button; }
