...
public partial class XYReferenceProperty
{
    internal static FuncControlTemplate<XYReferenceProperty> CreateTemplate() => new((control, scope) =>
    {
        ...
        var pickerContent = new ContentControl { Name = "PART_PickerContent" }; pickerContent[!ContentControl.ContentProperty] = control[!ReferencePickerContentProperty];
        ...
        var row = new Grid { Name = "PART_Row", ColumnDefinitions = new ColumnDefinitions("*,Auto"), Children = { label, field, popup } }; Grid.SetColumn(field, 1); control.LabelPart = label; control.ReferenceFieldPart = field; control.NamePart = name; control.IdentityPart = identity; control.LocatePart = locate; control.BrowsePart = browse; control.ClearPart = clear; control.PopupPart = popup;
    ...
    static XYIconButton Action(string partName, XyuiVectorIcon icon, string name, EventHandler<RoutedEventArgs> handler) { var button = new XYIconButton { Name = partName, Width = 28, Height = 28, Content = new XYIcon { Icon = icon, Size = XyuiIconSize.Small } }; button.SetValue(AutomationProperties.NameProperty, name); button.Click += handler; return button; }
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e) { base.OnApplyTemplate(e); LabelPart = e.NameScope.Find<TextBlock>("PART_Label") ?? LabelPart; ReferenceFieldPart = e.NameScope.Find<Border>("PART_ReferenceField") ?? ReferenceFieldPart; NamePart = e.NameScope.Find<TextBlock>("PART_Name") ?? NamePart; IdentityPart = e.NameScope.Find<TextBlock>("PART_Identity") ?? IdentityPart; LocatePart = e.NameScope.Find<XYIconButton>("PART_Locate") ?? LocatePart; BrowsePart = e.NameScope.Find<XYIconButton>("PART_Browse") ?? BrowsePart; ClearPart = e.NameScope.Find<XYIconButton>("PART_Clear") ?? ClearPart; PopupPart = e.NameScope.Find<Popup>("PART_Popup") ?? PopupPart; AttachPicker(); SyncParts(); }
}
