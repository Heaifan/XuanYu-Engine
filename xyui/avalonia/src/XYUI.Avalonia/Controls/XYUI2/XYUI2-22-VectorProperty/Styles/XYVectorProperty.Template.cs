...
public partial class XYVectorProperty
{
    internal static FuncControlTemplate<XYVectorProperty> CreateTemplate() => new((control, scope) =>
    {
        ...
        foreach (var axis in new[] { "X", "Y", "Z", "W" }) { var axisText = new TextBlock { Text = axis, HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center }; var field = new XYNumberField { Height = 30, HorizontalAlignment = HorizontalAlignment.Stretch, Background = Brushes.Transparent, BorderBrush = Brushes.Transparent, BorderThickness = new Thickness(0), CornerRadius = new CornerRadius(0), Tag = axis, IsScrubEnabled = true }; var axisCell = new Border { Classes = { "xyui-vector-axis-cell" }, Child = axisText }; var host = new Border { Classes = { "xyui-vector-axis-host" }, ClipToBounds = true, Child = new Grid { ColumnDefinitions = new ColumnDefinitions("25,*"), Children = { axisCell, field } } }; Grid.SetColumn(field, 1); axes.Children.Add(host); control.AxisFields.Add(field); control.AxisHosts.Add(host); field.PropertyChanged += control.OnAxisChanged; }
