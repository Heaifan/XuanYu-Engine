...
public partial class XYVectorProperty
{
    ...
    {
        var row = RowPart!; var axes = AxisPanelPart!;
        row.ColumnDefinitions.Clear(); row.RowDefinitions.Clear(); row.ColumnDefinitions.Add(new ColumnDefinition(GridLength.Star));
        row.RowDefinitions.Add(new RowDefinition(GridLength.Auto)); row.RowDefinitions.Add(new RowDefinition(GridLength.Auto));
        Grid.SetColumn(LabelPart!, 0); Grid.SetRow(LabelPart!, 0); Grid.SetColumn(axes, 0); Grid.SetRow(axes, 1); axes.Margin = new Thickness(0, 6, 0, 0);
    }
}
