...
public partial class XYNumberField
{
    internal static FuncControlTemplate<XYNumberField> CreateNumberTemplate() => new((control, scope) =>
    {
        ...
        var valueHost = new Border { Name = "PART_ValueHost", Padding = new Thickness(8, 0), Child = presenter };
        scope?.Register("PART_TextPresenter", presenter); scope?.Register("PART_ValueHost", valueHost);
        var suffix = new TextBlock { Name = "PART_Suffix", VerticalAlignment = VerticalAlignment.Center, Margin = new Thickness(0, 0, 10, 0), IsHitTestVisible = false };
        ...
        var content = new Grid { ColumnDefinitions = { new ColumnDefinition(1, GridUnitType.Star), new ColumnDefinition(GridLength.Auto), new ColumnDefinition(new GridLength(32)) }, Children = { valueHost, suffix, stepper } };
        Grid.SetColumn(suffix, 1); Grid.SetColumn(stepper, 2);
