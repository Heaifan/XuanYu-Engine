...
public partial class XYDatePicker
{
    internal static FuncControlTemplate<XYDatePicker> CreateTemplate() => new((control, scope) =>
    {
        ...
        var edge = new Border { Name = "PART_FocusEdge", Height = 3, VerticalAlignment = VerticalAlignment.Bottom, IsHitTestVisible = false }; var content = new ContentControl { Name = "PART_CalendarContent" }; var popupSurface = new Border { Name = "PART_CalendarSurface", Child = content }; var popup = new Popup { Name = "PART_Popup", Height = 0, IsVisible = false, Placement = PlacementMode.Bottom, IsLightDismissEnabled = true, Child = popupSurface }; var datePopupSurface = new Border { Name = "PART_DatePopupSurface" }; var datePopup = new Popup { Name = "PART_DatePopup", Height = 0, IsVisible = false, Placement = PlacementMode.Bottom, IsLightDismissEnabled = true, Child = datePopupSurface };
        control.CalendarContentPart = content; control.PopupPart = popup; control.CalendarSurfacePart = popupSurface; control.DatePopupPart = datePopup; control.DatePopupSurfacePart = datePopupSurface; datePopupSurface.Child = control.BuildDatePopup(); popupSurface.AddHandler(InputElement.PointerPressedEvent, control.OnCalendarSurfacePointerPressed, RoutingStrategies.Bubble, true); scope?.Register("PART_Popup", popup); scope?.Register("PART_CalendarContent", content); scope?.Register("PART_CalendarSurface", popupSurface); scope?.Register("PART_DatePopup", datePopup); scope?.Register("PART_DatePopupSurface", datePopupSurface);
        popup.Closed += (_, _) => control.CloseCalendarForLifecycle(); datePopup.Closed += (_, _) => control.OnDatePopupClosed(); return new Grid { Children = { chrome, edge, popup, datePopup } };
    });
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e) { base.OnApplyTemplate(e); PopupPart = e.NameScope.Find<Popup>("PART_Popup"); CalendarContentPart = e.NameScope.Find<ContentControl>("PART_CalendarContent"); DatePopupPart = e.NameScope.Find<Popup>("PART_DatePopup"); DatePopupSurfacePart = e.NameScope.Find<Border>("PART_DatePopupSurface"); RefreshCalendar(); RefreshDatePopup(); SyncDateParts(); if (IsCalendarOpen) OpenCalendar(); if (IsDatePopupOpen) OpenDatePopup(ActiveSegment); }
    Button CreateSegment(XYDateSegment segment, INameScope? scope) { var button = new Button { Name = $"PART_{segment}", Content = SegmentText(segment), Padding = new Thickness(2, 0), Template = XyuiControlStyles.ActionCellTemplate(), Background = Brushes.Transparent, BorderThickness = new Thickness(0), Focusable = true, MinWidth = segment == XYDateSegment.Year ? 38 : 26 }; button.Click += (_, _) => { if (ConsumePointerAction()) OpenDatePopup(segment); else { ActivateSegment(segment); OpenDatePopup(segment); } }; SegmentButtons[segment] = button; scope?.Register(button.Name!, button); return button; }
