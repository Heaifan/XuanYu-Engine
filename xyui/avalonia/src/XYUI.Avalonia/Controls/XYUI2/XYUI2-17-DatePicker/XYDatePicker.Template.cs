...
public partial class XYDatePicker
{
    internal static FuncControlTemplate<XYDatePicker> CreateTemplate() => new((control, scope) =>
    {
        ...
        calendar.Click += (_, _) => control.ToggleCalendar(); control.CalendarButtonPart = calendar; control.CalendarIconPart = calendar.Content as XYIcon; scope?.Register("PART_CalendarButton", calendar);
        var surface = new Grid { ColumnDefinitions = new ColumnDefinitions { new(32, GridUnitType.Pixel), new(1, GridUnitType.Star), new(32, GridUnitType.Pixel), new(32, GridUnitType.Pixel) }, Children = { previous, segments, next, calendar } }; surface.AddHandler(InputElement.PointerPressedEvent, control.OnSurfacePointerPressed, RoutingStrategies.Bubble, true);
        ...
        var edge = new Border { Name = "PART_FocusEdge", Height = 3, VerticalAlignment = VerticalAlignment.Bottom, IsHitTestVisible = false }; var content = new ContentControl { Name = "PART_CalendarContent" }; var popupSurface = new Border { Name = "PART_CalendarSurface", Child = content }; popupSurface.AddHandler(InputElement.PointerPressedEvent, control.OnCalendarSurfacePointerPressed, RoutingStrategies.Bubble, true); var popup = new Popup { Name = "PART_Popup", Height = 0, IsVisible = false, Placement = PlacementMode.Bottom, IsLightDismissEnabled = true, Child = popupSurface };
        control.CalendarContentPart = content; control.PopupPart = popup; control.CalendarSurfacePart = popupSurface; scope?.Register("PART_Popup", popup); scope?.Register("PART_CalendarContent", content); scope?.Register("PART_CalendarSurface", popupSurface);
        popup.Closed += (_, _) => control.CloseCalendarForLifecycle(); return new Grid { Children = { chrome, edge, popup } };
    });
    protected override void OnApplyTemplate(TemplateAppliedEventArgs e) { base.OnApplyTemplate(e); SegmentButtons.Clear(); PopupPart = e.NameScope.Find<Popup>("PART_Popup"); CalendarContentPart = e.NameScope.Find<ContentControl>("PART_CalendarContent"); RefreshCalendar(); SyncDateParts(); if (IsCalendarOpen) OpenCalendar(); }
