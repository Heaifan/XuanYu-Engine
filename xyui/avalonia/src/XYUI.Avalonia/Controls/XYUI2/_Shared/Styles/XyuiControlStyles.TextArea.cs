...
public static partial class XyuiControlStyles
{
    ...
    {
        var root = Input(styles, typeof(XYTextArea), "xyui-text-area", 54, false); root.Setters.Add(new Setter(TemplatedControl.TemplateProperty, XYTextArea.CreateTemplate())); Set(root, TemplatedControl.FontFamilyProperty, "XY.Font.UI"); Set(root, TemplatedControl.FontSizeProperty, "XY.FontSize.Body"); Set(root, TextBox.SelectionBrushProperty, "XY.Brush.Editor.Selection"); Set(root, TextBox.SelectionForegroundBrushProperty, "XY.Brush.Surface.Raised"); Set(root, TextBox.CaretBrushProperty, "XY.Brush.Accent.Strong");
        var header = new Style(x => x.OfType<XYTextArea>().Template().Name("PART_EditorHeader")); Set(header, Border.BackgroundProperty, "XY.Brush.Surface.PanelAlt"); Set(header, TemplatedControl.ForegroundProperty, "XY.Brush.Text.Secondary"); styles.Add(header);
        ...
        var metadata = new Style(x => x.OfType<XYTextArea>().Template().Name("PART_EditorMetadata")); Set(metadata, TemplatedControl.ForegroundProperty, "XY.Brush.Text.Secondary"); styles.Add(metadata);
        var modified = new Style(x => x.OfType<XYTextArea>().Template().Name("PART_EditorModified")); Set(modified, TemplatedControl.ForegroundProperty, "XY.Brush.Text.Secondary"); styles.Add(modified);
        var body = new Style(x => x.OfType<XYTextArea>().Template().Name("PART_EditorBody")); Set(body, Border.BackgroundProperty, "XY.Brush.Surface.Input"); styles.Add(body);
        ...
        State(styles, typeof(XYTextArea), "xyui-text-area", ":disabled", TemplatedControl.BackgroundProperty, "XY.Brush.State.Disabled.Background"); State(styles, typeof(XYTextArea), "xyui-text-area", ":disabled", TemplatedControl.ForegroundProperty, "XY.Brush.State.Disabled.Text"); State(styles, typeof(XYTextArea), "xyui-text-area", ":disabled", TemplatedControl.BorderBrushProperty, "XY.Brush.State.Disabled.Border"); State(styles, typeof(XYTextArea), "xyui-text-area", ":error", TemplatedControl.BorderBrushProperty, "XY.Brush.Semantic.Error.Border");
