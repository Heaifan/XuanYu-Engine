...
public static partial class XyuiControlStyles
{
    static void SearchField(Styles styles)
    {
        ...
        var filterCell = new Style(x => x.OfType<XYSearchField>().Template().Name("PART_FilterCell")); filterCell.Setters.Add(new Setter(Button.TemplateProperty, ActionCellTemplate())); Set(filterCell, Button.BackgroundProperty, "XY.Brush.Surface.PanelAlt"); styles.Add(filterCell);
        ...
        var filterSurface = new Style(x => x.OfType<XYSearchField>().Template().Name("PART_FilterSurface")); Set(filterSurface, Border.BackgroundProperty, "XY.Brush.Surface.Raised"); Set(filterSurface, Border.BorderBrushProperty, "XY.Brush.Border.Color.Default"); filterSurface.Setters.Add(new Setter(Border.BorderThicknessProperty, new Thickness(1))); filterSurface.Setters.Add(new Setter(Border.CornerRadiusProperty, new CornerRadius(3))); styles.Add(filterSurface);
    ...
    static void PasswordField(Styles styles)
    {
        ...
        IconPart(styles, typeof(XYPasswordField), "PART_EyeGlyph", "XY.Brush.Text.Secondary"); PasswordPart(styles, "PART_Placeholder", "XY.Brush.Text.Placeholder"); var cell = new Style(x => x.OfType<XYPasswordField>().Template().Name("PART_RevealCell")); cell.Setters.Add(new Setter(Button.TemplateProperty, ActionCellTemplate())); Set(cell, Button.BackgroundProperty, "XY.Brush.Surface.PanelAlt"); styles.Add(cell);
    ...
    internal static FuncControlTemplate<Button> ActionCellTemplate() => new((button, _) => { var chrome = new Border { ClipToBounds = true }; chrome[!Border.BackgroundProperty] = button[!TemplatedControl.BackgroundProperty]; chrome[!Border.BorderBrushProperty] = button[!TemplatedControl.BorderBrushProperty]; chrome[!Border.BorderThicknessProperty] = button[!TemplatedControl.BorderThicknessProperty]; chrome[!Border.CornerRadiusProperty] = button[!TemplatedControl.CornerRadiusProperty]; var presenter = new ContentPresenter { HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center }; presenter[!ContentPresenter.ContentProperty] = button[!Button.ContentProperty]; presenter[!ContentPresenter.PaddingProperty] = button[!TemplatedControl.PaddingProperty]; presenter[!ContentPresenter.HorizontalContentAlignmentProperty] = button[!Button.HorizontalContentAlignmentProperty]; presenter[!ContentPresenter.VerticalContentAlignmentProperty] = button[!Button.VerticalContentAlignmentProperty]; presenter[!TextElement.ForegroundProperty] = button[!TemplatedControl.ForegroundProperty]; chrome.Child = presenter; return chrome; });
