...
public sealed class MapGeometryContextHitTesterTests
{
    ...
    [Fact]
    public void Edge_hit_keeps_exact_projected_world_point()
    {
        var map = MapDefaultDefinition.CreateDefault();
        var road = new MapRoad(MapRoadId.New(), map.ActiveLayer(), "道路", "generic", [new(-100, 0), new(100, 0)]);
        map = map with { Roads = [road] }; var projection = Projection();
        var edge = projection.ProjectWorldPoint(new Vector3d(25, 0, 0));

        Assert.True(MapGeometryContextHitTester.TryHit(map, projection, edge.X, edge.Y + 5, 0, out var hit));
        Assert.Equal(MapGeometryContextKind.Edge, hit.Kind);
        Assert.NotNull(hit.ProjectedPoint);
        Assert.InRange(hit.ProjectedPoint!.Value.X, 24.9, 25.1);
        Assert.Equal(0, hit.ProjectedPoint!.Value.Y, 6);
    }

    [Fact]
    public void Face_hit_uses_topmost_region_layer()
    {
        var map = MapDefaultDefinition.CreateDefault(); var lower = map.ActiveLayer();
        var upper = MapLayerStack.CreateRegionLayer(map.Layers, "上层");
        var first = new MapRegion(MapRegionId.New(), lower, "下层", MapRegionKind.Generic,
            [new(-100, -100), new(100, -100), new(100, 100), new(-100, 100)]);
        var second = new MapRegion(MapRegionId.New(), upper.LayerId, "上层", MapRegionKind.Generic,
            [new(-100, -100), new(100, -100), new(100, 100), new(-100, 100)]);
        var projection = Projection(); var center = projection.ProjectWorldPoint(new Vector3d(0, 0, 0));

        Assert.True(MapGeometryContextHitTester.TryHit(map with { Layers = map.Layers.Add(upper), Regions = [first, second] },
            projection, center.X, center.Y, 0, out var hit));
        Assert.Equal(second.RegionId.ToString(), hit.Selection.FeatureId);
    }
