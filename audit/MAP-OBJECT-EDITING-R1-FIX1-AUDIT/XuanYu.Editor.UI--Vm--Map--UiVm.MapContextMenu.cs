...
public readonly record struct MapGeometryContextRequest(MapGeometryContextHit Hit, double X, double Y);

public sealed partial class UiVm
{
    public event Action<MapGeometryContextRequest>? MapGeometryContextRequested;
    ...
    public bool PublishMapGeometryContext(EditorPointerEvent pointer)
    {
        ...
        MapGeometryContextRequested?.Invoke(new(hit, pointer.Position.X, pointer.Position.Y));
        return true;
    }
    ...
    {
        if (hit.Kind != MapGeometryContextKind.Edge || hit.ProjectedPoint is not { } point)
            return XuanYu.Core.Results.EngineResult.Fail(XuanYu.Core.Results.EngineError.Create("InvalidEdgePoint", "当前边没有有效投影点。"));
        return hit.Selection.Kind switch
        {
            MapGeometryFeatureKind.Region => MapSession.InsertRegionVertex(MapRegionIdFrom(hit.Selection), hit.SegmentIndex, point),
            MapGeometryFeatureKind.Road => MapSession.InsertRoadVertex(MapRoadIdFrom(hit.Selection), hit.SegmentIndex, point),
            _ => XuanYu.Core.Results.EngineResult.Fail(XuanYu.Core.Results.EngineError.Create("InvalidEdge", "当前对象不是可插点的线性几何。"))
        };
    }
