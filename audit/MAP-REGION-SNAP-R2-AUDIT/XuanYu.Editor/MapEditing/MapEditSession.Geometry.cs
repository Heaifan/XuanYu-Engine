...
public sealed partial class MapEditSession
{
    ...
    public EngineResult InsertRoadVertex(MapRoadId roadId, int segmentIndex, MapPoint point)
    {
        if (!GuardWriteThread()) return Fail("NotOnWriteThread", "新增道路节点必须在编辑写线程执行。");
        var road = _currentMap.Roads.FirstOrDefault(item => item.RoadId == roadId);
        if (road is null) return Fail("UnknownRoad", "道路不存在。");
        if (MapLayerRules.Find(_currentMap.Layers, road.LayerId)?.IsLocked == true)
            return Fail("RoadLayerLocked", "道路所属图层已锁定。");
        if (segmentIndex < 0 || segmentIndex >= road.Points.Length - 1)
            return Fail("InvalidRoadSegment", "新增节点必须位于有效道路线段上。");
        return EditRoadPoints(roadId, road.Points.Insert(segmentIndex + 1, point));
    }

    public EngineResult DeleteRoadVertex(MapRoadId roadId, int vertexIndex)
    {
        if (!GuardWriteThread()) return Fail("NotOnWriteThread", "删除道路节点必须在编辑写线程执行。");
        var road = _currentMap.Roads.FirstOrDefault(item => item.RoadId == roadId);
        if (road is null) return Fail("UnknownRoad", "道路不存在。");
        if (road.Points.Length <= 2) return Fail("RoadMinimumVertices", "道路至少保留两个节点。");
        if (vertexIndex < 0 || vertexIndex >= road.Points.Length)
            return Fail("InvalidRoadVertex", "道路节点索引无效。");
        return EditRoadPoints(roadId, road.Points.RemoveAt(vertexIndex));
    }
