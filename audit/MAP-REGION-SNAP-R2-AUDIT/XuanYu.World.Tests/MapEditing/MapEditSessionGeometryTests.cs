...
public sealed class MapEditSessionGeometryTests
{
    ...
    [Fact]
    public void Road_vertex_insert_is_exact_and_undoable()
    {
        var session = new MapEditSession();
        var road = new MapRoad(MapRoadId.New(), session.ActiveRegionLayerId, "道路", "generic",
        ...
        Assert.True(session.InsertRoadVertex(road.RoadId, 0, new(50, 0)).IsSuccess);
        Assert.Equal([new(0, 0), new(50, 0), new(100, 0), new(100, 100)],
            session.CurrentMap.Roads[0].Points.ToArray());
        Assert.True(session.Undo().IsSuccess);
        Assert.Equal(road.Points.ToArray(), session.CurrentMap.Roads[0].Points.ToArray());
    }

    [Fact]
    public void Road_vertex_delete_is_exact_and_cannot_break_line()
    {
        var session = new MapEditSession();
        var road = new MapRoad(MapRoadId.New(), session.ActiveRegionLayerId, "道路", "generic",
        ...
        Assert.True(session.DeleteRoadVertex(road.RoadId, 1).IsSuccess);
        Assert.Equal([new(0, 0), new(100, 0)], session.CurrentMap.Roads[0].Points.ToArray());
        Assert.True(session.DeleteRoadVertex(road.RoadId, 0).IsFailure);
        Assert.Equal(2, session.CurrentMap.Roads[0].Points.Length);
    }
}
