...
public sealed partial class VulkanNativeHost
{
    ...
    void OnNativePointerMessage(NativePointerMessage message)
    {
        ...
        if (message.Message == NativePointerMessage.RightDown && DataContext is UiVm contextVm &&
            contextVm.TryGetMapGeometryContext(x, y, contextVm.CurrentViewport, out var hit))
            ShowMapGeometryContextMenu(contextVm, hit, x, y);
