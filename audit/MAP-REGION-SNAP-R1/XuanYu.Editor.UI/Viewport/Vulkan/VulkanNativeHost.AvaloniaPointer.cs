...
public sealed partial class VulkanNativeHost
{
    protected override void OnPointerPressed(PointerPressedEventArgs e)
    {
        ...
        RememberRegionSnapPointer(point.Position.X, point.Position.Y);
        ...
        if (vm.TrySelectMapGeometryVertex(point.Position.X, point.Position.Y, CaptureViewportState()))
        { e.Handled = true; return; }
        if (ReportDrawing(vm, point.Position.X, point.Position.Y, e.KeyModifiers.HasFlag(KeyModifiers.Alt)))
        {
            e.Handled = true; return;
        }
