...
public static class RegionVertexSnapResolver
{
    ...
    {
        settings.Validate();
        if (state.IsSnapped && TryKeep(state, pointerX, pointerY, map, projection, regionLookup, settings, out var heldDistance))
            return new(state.TargetPoint, true, state.TargetRegionId, state.TargetVertexIndex, heldDistance);
        state.Clear();
        ...
        return new(target.Point, true, target.RegionId, target.VertexIndex, Math.Sqrt(target.Distance));
    }
    ...
    {
        screenDistance = double.PositiveInfinity;
        ...
        if (distance > settings.ReleaseRadiusPx * settings.ReleaseRadiusPx) return false;
        screenDistance = Math.Sqrt(distance);
