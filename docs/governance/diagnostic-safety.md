...
高频链路只能写入快照、限流摘要、重复折叠计数或临时探针缓冲区。

### 后台任务日志规则

后台任务可以产生低频事实日志，但不得直接修改 UI 绑定集合。

以下来源未来接入日志时必须先进入日志队列或调度边界：
...
```
后台线程不得直接触碰 Avalonia 绑定集合
后台线程不得逐条 Dispatcher.UIThread.Invoke
后台线程应写入线程安全日志队列或低频摘要
UI 层通过 Dispatcher.UIThread.Post 合批刷新
刷新失败不得阻塞后台任务
```

当前 `XuanYu.Editor.UI` 的 `EditorLogBus` 只接 UI 命令、工具切换和启动提示等低频 UI 事件。后续接入后台构建、导入或渲染摘要前，必须先补线程安全队列与合批刷新边界。

### 日志详情选择规则

日志详情面板只能由用户点击日志行后更新选中项。
...
原因：日志列表和详情区属于普通 UI 日志视图，不应在鼠标移动期间制造额外绑定刷新。需要查看详情时，用户必须点击日志行进行选择。
...
| 日期 | 修订人 | 说明 |
|---|---|---|
| 2026-07-06 | — | RZ-Fix1-E-R1：补充底部普通日志准入和高频事件禁止规则 |
