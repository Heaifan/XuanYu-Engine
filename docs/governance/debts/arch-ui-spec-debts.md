...
- **状态**：**治理中**（2026-08-06 更新：D0 COMPLETE / D1 COMPLETE / D2 COMPLETE / D3 COMPLETE；**D4 检查器/地图编辑器/图层工作面板治理 READY FOR USER ACCEPTANCE**（待真机裁决））
...
- **当前追踪**：
  ...
  - **D4 完成（2026-08-06，READY FOR USER ACCEPTANCE）**：检查器（字号 Token 层级/响应式双模式 360/结构化字段/全宽分组去卡片/调试页 96 列）；地图编辑器（只读摘要 72 列/MapId 前 8…后 6+Tooltip+复制/紧凑模式 320/单滚动/按钮组 6）；图层面板（图标 16/热区 26×24/Layer.* Token/选中样式/三重区分/插入线 DropLine #5B8DB8）；**基线 226 → 159（-67，真实迁移）**；守卫缺陷修复（warcore $failures 重置吞失败 + 5+100 self-check 期望笔误）；保留项：环境页内容补齐（D5）、日志/弹窗/状态（D5）、焦点/DPI（D6）；
  ...
  - 下一步：**D4 真机验收（IPO 清单组 1~18 + 补充 19~21）→ D5 状态/表单/弹窗/通知/日志治理**；
