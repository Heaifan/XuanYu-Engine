...
- **状态**：**治理完成**（2026-08-09 更新：D0/D1/D2/D3/D4/D5/D6 COMPLETE；ARCH-UI-SPEC-R1 已随 MAP-A-R2 收口。**D5-DEFER-01：地图「保存并新建」暂缓**——转入 MAP-A-R3 地图持久化候选，不归入 R2。专项必须补：保存成功后才新建；保存失败不新建；取消路径选择不新建；保存期间防重复提交；保存成功后更新路径和状态；写盘失败时保留地图、图层和历史。）
