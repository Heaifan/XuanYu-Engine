...
- **状态**：**治理中**（2026-08-06 更新：D0/D1/D2/D3/D4/D5 COMPLETE；当前阶段 **ARCH-UI-SPEC-R1-D6**。**D5-DEFER-01：地图「保存并新建」暂缓**——归属未来独立的地图持久化专项（不归入 D6）。专项必须补：保存成功后才新建；保存失败不新建；取消路径选择不新建；保存期间防重复提交；保存成功后更新路径和状态；写盘失败时保留地图、图层和历史。）
...
- **当前追踪**：
  ...
  - **D6 剩余项**：DPI/缩放、键盘与基础可访问性、减少动画偏好、UI/日志性能、剩余旧 UI 债务清零复核；D6 后进入 ARCH-UI-SPEC-R1-A1 总验收；
