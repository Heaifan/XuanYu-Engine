...
状态：`READY FOR USER RE-ACCEPTANCE`，不是 `CLOSED`。R2-F1 先复验 M02/M03，再按 R2-F2 的自动 ID 与中文列表路径复验 M02/M03，随后继续 M04～M07；实现边界止于 Dataset Registry 与最小 Dataset 页面；Geometry、Feature 内容、properties、AI 编辑 API、Asset Reference、Layer/Renderer/Picking 重构均不在本轮。
...
自动证据：C1 `14/14`、C2 `10/10`、C3 `8/8`、C4 UI `7/7`、F2 focused `17/17` PASS；自动测试不替代上述真机结论。
