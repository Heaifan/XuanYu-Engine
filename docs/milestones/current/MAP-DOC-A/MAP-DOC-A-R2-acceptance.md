...
| R2-M02 | 数据集 → 新建 | ID `roads`、类型 `road` | 点击新建数据集 | 生成 `data/roads.json`，Manifest 出现注册项；待真机填写 |
| R2-M03 | 数据集列表 | 已创建 Dataset | 观察列表 | 显示 Type、ID、Status=正常；待真机填写 |
...
自动证据：C1 `14/14`、C2 `10/10`、C3 `8/8`、C4 UI `7/7` focused PASS；自动测试不替代上述真机结论。
