...
- GRID-RW-2A：独立 World Reference Grid 骨架已实现：Fullscreen Triangle + World XY（Z=0）求交 + 固定 100m；DepthTest/DepthWrite 关闭，MapGround ON/OFF 不影响网格存在。等待真机只验独立存在与缩放不消失；RW-2B（全帧 Step）、RW-2C（Analytical AA）与 RW-2D（视觉层级）均 BLOCKED。
