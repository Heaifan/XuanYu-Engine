...
基线：`6c817ae`；最新用户回归裁定日期：2026-08-12；整体：不通过。

回归失败证据：进入“区域编辑 → 绘制区域”，不落点并移动鼠标，或移动到已有顶点准备拖动时，Editor 发生闪退；原因与修复记录转入 `MAP-DATA-A-R2-F2 · Region Pointer Safety`。
...
总体：不通过（Region Pointer Safety 回归；F2 修复并真机通过后再回到本清单复验）
