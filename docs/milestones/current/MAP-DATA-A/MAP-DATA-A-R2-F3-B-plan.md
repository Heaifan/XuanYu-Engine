...
状态：`CLOSED`；F3-A 已 CLOSED。本轮只把 F2 Region 顶点拖拽接入 F3-A 局部查询，不重写拖拽、不实现 Edge Snap 或 Road Snap。
...
专项覆盖 17 项核心行为及 10,000 候选结构烟测：滞回、最近点、稳定决胜、当前区域排除、局部 bounds、查询失败、F2 接线、状态清理和候选工作集边界。2026-08-13 完整正式门禁通过；用户真机 M01～M10 全部 PASS，正式 `CLOSED`。
