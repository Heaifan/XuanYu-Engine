...
### 8.7.8B-2 — VulkanSurfaceProbe SRP 拆分

将 `VulkanSurfaceProbe`（203 行）拆成 2 个小文件，不改外部 API 签名：

#### 操作

| 文件 | 行数 | 职责 |
|------|------|------|
| `VulkanSurfaceProbe.cs` | 66 | 薄门面 — 参数校验、编排探针流程、包装诊断结果 |
| `VulkanSurfaceInstanceScope.cs` | 98 | 临时 VkInstance 生命周期 + Win32 Surface 创建（IDisposable） |

Scope 完全隐藏在 `VulkanSurfaceProbe.ProbeWindows(...)` 内部，调用方无需感知。
...
| 生产文件 ≤100 行 | ✅ 全部达标 |
| 目录文件数 (Surface/) | 4（含 Info + Status）≤5 ✅ |
| 白名单删除 | ✅ `VulkanSurfaceProbe.cs` 移出白名单 |
| 债务路线图 | ✅ 已更新 |

---

### 8.7.8B-4 — VulkanDeviceProbe SRP 拆分

将 `VulkanDeviceProbe`（288 行）拆成 3 个小文件，不改 `Probe()` 签名：

#### 操作

| 文件 | 行数 | 职责 |
|------|------|------|
| `VulkanDeviceProbe.cs` | 77 | 薄门面 — Probe() 编排 + LogicalDevice 创建 + 诊断包装 |
| `VulkanDeviceInstanceScope.cs` | 61 | 临时 VkInstance 生命周期（无扩展，IDisposable） |
| `VulkanDeviceSelector.cs` | 80 | PhysicalDevice 枚举 + 选择 + Queue 族检查 |

InstanceScope 与 Selector 完全隐藏在 `VulkanDeviceProbe.Probe()` 内部，调用方无需感知。
