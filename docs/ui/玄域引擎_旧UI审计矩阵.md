...
| G03 | 检查器响应式双模式 | 检查器字段为 ListBox 渲染（InspectorFields），无「宽=左右、窄=上下」切换逻辑 | 切换阈值 内容宽 <360 DIP 整组切换；标签列 96；字段最小 128 | D4（D4 已清零：InspectorPanel 宽/窄双布局树，同一 InspectorFields 数据源；纯逻辑 InspectorLayoutModel 阈值 360） |
| G04 | 面板紧凑/折叠 | 无紧凑模式切换、无面板折叠与恢复入口 | 内容宽 <320 进紧凑；折叠次要区/隐藏低优先级面板且保留恢复入口 | D3（D4 部分落地：地图页紧凑模式 <320 已实现——整组上下+减少留白+关键操作保留；面板折叠/恢复入口留后续轮） |
...
| D4 | 右侧面板/检查器/图层 | W44、W48、W53、G03（**D4 已清零**）+ W37-W42、W45-W52、W54、W14（D4 文件内 Token 迁移清零） |
