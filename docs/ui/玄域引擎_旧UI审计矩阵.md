...
| D5 | 状态/表单/菜单/弹窗/日志 | W36、W59、W68、G05、G08（**D5 二次纠偏后 READY FOR USER RE-ACCEPTANCE**：未保存判断 = IsDirty || 待提交表单（默认地图基线 MarkBaseline）；输入阶段真实校验（错误不消失/输入中态/明显超界，边界与领域一致）；「保存并新建」停止上报（无真实地图持久化，D6 恢复）；G02 焦点系统完整化/自动消失策略/地图持久化留 D6） |
