...
| 文件 | 行数 | 为什么难拆 |
|------|------|-----------|
| `EditorShell.axaml.cs` | 491 | **⚠️ 组合根例外** — 不可删除白名单。原因见 `docs/audit-EditorShell-closeout-8.7.8H-5.md`。Shell 只保留装配/字段/回调连接，后续不得新增业务逻辑 |
...
当前产线白名单预算：49（9 生产 + 20 测试 + 20 松弛，B 类 3 个 + G-2 + F-2 全部移出白名单）
EditorShell 白名单保留中（491 行，**组合根例外** — 不可删除，见 H-5 收口审计）
测试白名单：20 项（保留）
