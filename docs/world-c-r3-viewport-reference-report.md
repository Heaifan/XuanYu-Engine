...
`WORLD-C-R3-R4：真机视觉修复轮，等待真机重新验收`。

本轮在 R3-R3 真机验收 FAIL 基线上继续修复：Scale Gizmo 在无可见 Global/Local 切换入口时锁定世界坐标轴；顶部命令栏、菜单和未保存确认框继续向已确认 SVG 视觉方案收敛。交互、命中、拖动、提交、撤销和场景文档合同保持不变。
