...
`WORLD-C-R3-R5：真机视觉修复轮，等待真机重新验收`。

本轮在 R3-R4 真机复测 FAIL 基线上继续修复：Move Gizmo 不再显示三块抢视觉的平面正方形；平面拖动只保留中心附近的小型低存在感角标。交互、命中、拖动、提交、撤销和场景文档合同保持不变。
...
- Move：细轴 + 箭头 + 中性中心手柄；XY/XZ/YZ 平面不再是大正方形，只保留小型低饱和三角角标。
