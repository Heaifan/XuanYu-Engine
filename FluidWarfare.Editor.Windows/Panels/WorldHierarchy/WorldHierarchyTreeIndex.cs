...
{
    public Dictionary<string, WorldHierarchyNodeView>
        NodeViewsById { get; } = [];

    public Dictionary<string, WorldHierarchyNodeView>
        EntityViewsById { get; } = [];

    public Dictionary<string, List<WorldHierarchyNodeView>>
        AncestorViewsByEntityId { get; } = [];

    public static (
        List<WorldHierarchyNodeView> Roots,
        WorldHierarchyTreeIndex Index)
        Build(WorldHierarchyTree tree)
    {
        ...
        bool[] ancestorHasNext)
    {
        ...
        {
            index.EntityViewsById[view.EntityId] = view;
            index.AncestorViewsByEntityId[view.EntityId] =
                [.. ancestorPath];
        }
        ...
        bool[] ancestorHasNext)
    {
        for (var i = 0; i < node.Children.Count; i++)
        {
            ...
            var childPath =
                new List<WorldHierarchyNodeView>(ancestorPath)
                {
                    view
                };
