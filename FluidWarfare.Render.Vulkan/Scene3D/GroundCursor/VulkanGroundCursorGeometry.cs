...
public static class VulkanGroundCursorGeometry
{
    /// <summary>
    /// 生成 Ground Cursor 顶点。
    /// 青色 (0.0, 1.0, 1.0, 1.0)，高度偏移 0.02 避免与网格 Z-fighting。
    /// 视觉范围约 0.8 × 0.8 米。
    /// </summary>
    public static VulkanScene3dVertex[] Create()
    {
        const float size = 0.4f;          // 半边长
        const float cx = 0f, cz = 0f;     // 局部中心
        ...
        [
            // ─── 中心十字（4 条线段 = 8 个顶点） ───────────────────
            // X 方向横线
            new VulkanScene3dVertex(cx - size, 0, cz, r, g, b, a),
            new VulkanScene3dVertex(cx + size, 0, cz, r, g, b, a),
            // Z 方向竖线
            new VulkanScene3dVertex(cx, 0, cz - size, r, g, b, a),
            new VulkanScene3dVertex(cx, 0, cz + size, r, g, b, a),

            // ─── 外部小方框（4 条线段 = 8 个顶点） ─────────────────
            // 上边
            new VulkanScene3dVertex(cx - size * 0.55f, 0, cz - size * 0.55f, r, g, b, a),
            new VulkanScene3dVertex(cx - size * 0.55f, 0, cz + size * 0.55f, r, g, b, a),
            // 右边
            new VulkanScene3dVertex(cx - size * 0.55f, 0, cz + size * 0.55f, r, g, b, a),
            new VulkanScene3dVertex(cx + size * 0.55f, 0, cz + size * 0.55f, r, g, b, a),
