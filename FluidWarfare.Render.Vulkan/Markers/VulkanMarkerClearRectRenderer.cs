...
/// <summary>
/// 使用 vkCmdClearAttachments 在 Vulkan 视口中绘制点位小方块。
/// 不创建 Shader/Pipeline/Mesh/Texture/Buffer。
/// 全链路探测模式：Instance → Surface → Device → Swapchain → 清屏+点位 → Present → 清理。
/// 遵循 7.8.1/7.8.2 已验证的指针参数 delegate 模式，避免 0xC0000005。
/// </summary>
public static unsafe class VulkanMarkerClearRectRenderer
{
    /* 背景清屏颜色：深蓝色，与 ClearProbe 一致 */
    ...
    /* 点位颜色：浅黄色 */
    private const float MarkerR = 1.00f;
    private const float MarkerG = 0.82f;
    private const float MarkerB = 0.20f;
    private const float MarkerA = 1.00f;

    /// <summary>
    /// 执行一次完整的渲染链路：清屏 + 点位绘制 + Present。
    /// 如果 markerInfo 为空，只执行清屏（回退为纯清屏探测）。
    /// </summary>
    /// <param name="hinstance">Windows 实例句柄。</param>
    /// <param name="hwnd">Windows 窗口句柄。</param>
    /// <param name="reqW">请求宽度。</param>
    /// <param name="reqH">请求高度。</param>
    /// <param name="markerInfo">点位信息，可为 null。</param>
    public static VulkanMarkerDrawResult RenderWindows(
        nint hinstance, nint hwnd, uint reqW, uint reqH, VulkanMarkerDrawInfo? markerInfo)
    {
        ...
        try
        {
            ...
            // Surface 函数：GetInstanceProcAddr
            ...
            // Swapchain 设备函数：GetDeviceProcAddr
            ...
            // --- 可选：点位绘制 (vkCmdClearAttachments) ---
            if (markerInfo is not null)
            {
                // 限制点位在视口范围内
                var clampedX = Math.Clamp(markerInfo.PixelX - markerInfo.PixelSize / 2, 0, (int)(extent.Width - 1));
                var clampedY = Math.Clamp(markerInfo.PixelY - markerInfo.PixelSize / 2, 0, (int)(extent.Height - 1));
                var markerW = (uint)Math.Min(markerInfo.PixelSize, (int)extent.Width);
                var markerH = (uint)Math.Min(markerInfo.PixelSize, (int)extent.Height);

                var markerClearVal = new ClearValue
                {
                    Color = new ClearColorValue
                    {
                        Float32_0 = MarkerR,
                ...
                var clearAttachment = new ClearAttachment
                {
                    AspectMask = ImageAspectFlags.ColorBit,
                    ColorAttachment = 0,
                    ClearValue = markerClearVal
                };

                var clearRect = new ClearRect
                {
                    Rect = new Rect2D(
                        new Offset2D(clampedX, clampedY),
                        new Extent2D(markerW, markerH)),
                    BaseArrayLayer = 0,
                    LayerCount = 1
                };

                vk.CmdClearAttachments(cmdBuf, 1, &clearAttachment, 1, &clearRect);
                drawnCount = 1;
            }
            ...
            var markerMsg = drawnCount > 0
                ? $"Vulkan 清屏 + 点位绘制成功，点位数量：{drawnCount}，颜色：rgba({MarkerR:F2}, {MarkerG:F2}, {MarkerB:F2}, {MarkerA:F2})，尺寸：{extent.Width}x{extent.Height}，用时：{sw.Elapsed.TotalMilliseconds:F2} ms。"
                : $"Vulkan 清屏成功（无点位），尺寸：{extent.Width}x{extent.Height}，用时：{sw.Elapsed.TotalMilliseconds:F2} ms。";

            var result = new VulkanMarkerDrawResult(
                VulkanMarkerDrawStatus.Succeeded,
                markerMsg,
                drawnCount,
    ...
    private static bool CreateInstance(Vk vk, out Silk.NET.Vulkan.Instance inst)
    {
        ...
        try
        {
            ...
            var ai = new ApplicationInfo
            {
                ...
                PApplicationName = (byte*)a,
                ApplicationVersion = 1,
                PEngineName = (byte*)e,
                EngineVersion = 1,
                ApiVersion = PackVer(1, 0, 0)
            };
    ...
    private static bool CreateDevice(Vk vk, Silk.NET.Vulkan.PhysicalDevice pd, uint qi,
        out Silk.NET.Vulkan.Device dev)
    {
        dev = default;
        var qp = 1.0f;
    ...
    private static VulkanMarkerDrawResult Fail(string msg, Stopwatch sw) =>
        new(VulkanMarkerDrawStatus.Failed, msg, 0, sw.Elapsed.TotalMilliseconds);
    ...
    private delegate Result CreateWin32SurfacePtr(Silk.NET.Vulkan.Instance i, Win32SurfaceCreateInfoKHR* ci,
        AllocationCallbacks* a, SurfaceKHR* s);

    [UnmanagedFunctionPointer(CallingConvention.Winapi)]
    private delegate void DestroySurfacePtr(Silk.NET.Vulkan.Instance i, SurfaceKHR s, AllocationCallbacks* a);

    [UnmanagedFunctionPointer(CallingConvention.Winapi)]
    private delegate Result SurfaceSupportPtr(Silk.NET.Vulkan.PhysicalDevice pd, uint qi, SurfaceKHR s, int* supported);

    [UnmanagedFunctionPointer(CallingConvention.Winapi)]
    private delegate Result GetCapsPtr(Silk.NET.Vulkan.PhysicalDevice pd, SurfaceKHR s, SurfaceCapabilitiesKHR* c);

    [UnmanagedFunctionPointer(CallingConvention.Winapi)]
    private delegate Result GetFormatsPtr(Silk.NET.Vulkan.PhysicalDevice pd, SurfaceKHR s, uint* c,
        SurfaceFormatKHR* f);
    ...
    [UnmanagedFunctionPointer(CallingConvention.Winapi)]
    private delegate Result CreateSwapchainPtr(Silk.NET.Vulkan.Device d, SwapchainCreateInfoKHR* ci,
        AllocationCallbacks* a, SwapchainKHR* sc);

    [UnmanagedFunctionPointer(CallingConvention.Winapi)]
    private delegate void DestroySwapchainPtr(Silk.NET.Vulkan.Device d, SwapchainKHR sc, AllocationCallbacks* a);

    [UnmanagedFunctionPointer(CallingConvention.Winapi)]
    private delegate Result GetSwapchainImagesPtr(Silk.NET.Vulkan.Device d, SwapchainKHR sc, uint* c, Image* imgs);

    [UnmanagedFunctionPointer(CallingConvention.Winapi)]
    private delegate Result AcquireNextImagePtr(Silk.NET.Vulkan.Device d, SwapchainKHR sc, ulong t,
        Silk.NET.Vulkan.Semaphore s, Fence f, uint* i);

    [UnmanagedFunctionPointer(CallingConvention.Winapi)]
    private delegate Result QueuePresentPtr(Queue q, PresentInfoKHR* p);
}
