...
unsafe sealed class VulkanCommandPool : IDisposable
{
    ...
    public void Clear(Image image, ImageLayout oldLayout, ImageLayout newLayout)
    {
        ...
        var color = new ClearColorValue(0.08f, 0.27f, 0.38f, 1f); _api.CmdClearColorImage(command, image, ImageLayout.TransferDstOptimal, in color, 1, in barrier.SubresourceRange); _api.EndCommandBuffer(command).Ensure();
        var submit = new SubmitInfo { SType = StructureType.SubmitInfo, CommandBufferCount = 1, PCommandBuffers = &command }; _api.QueueSubmit(_queue, 1, in submit, default).Ensure(); _api.QueueWaitIdle(_queue).Ensure(); _api.FreeCommandBuffers(_device, _pool, 1, in command);
    }
