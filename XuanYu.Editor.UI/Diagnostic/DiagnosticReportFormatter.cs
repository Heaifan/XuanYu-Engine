...
public static class DiagnosticReportFormatter
{
    ...
    public static string FormatAi(DiagnosticElementSnapshot s) => Lines(
        ...
        "五、诊断信息", "", $"XYUI映射：{(s.Identity.IsMapped ? "成功" :
            s.Identity.Source == "XYUI" ? "未登记" : "不适用")}",
        "自动异常检测：未执行");

    static string FormatIdentity(DiagnosticElementSnapshot s) => Lines(
        $"XYUI索引：{s.Identity.DisplayIndex}", $"XYUI编号：{Number(s)}",
    ...
    static string Layout(DiagnosticElementSnapshot s) => Lines(
        ...
        $"垂直对齐：{s.VerticalAlignment}", "裁剪状态：不可用", "滚动容器：不可用", "层级：不可用");
    ...
    static string Number(DiagnosticElementSnapshot s) => string.IsNullOrEmpty(s.Identity.Number) ? "未登记" : s.Identity.Number;
